...
<p align="center">
  <picture>
    ...
    <img src="https://raw.githubusercontent.com/Chirudeva-Reddy/Chirudeva-Reddy/output/ocean.svg" alt="Ocean-themed animation of the GitHub contribution snake" />
  </picture>
</p>
...
I lift most days, so the same workflow that draws everything else plots it. Weekly minutes over the last 26 weeks.
...
<picture>
  ...
  <img width="100%" src="https://raw.githubusercontent.com/Chirudeva-Reddy/Chirudeva-Reddy/output/training.svg" alt="Weekly training minutes over the last 26 weeks, with session count, week streak and average session length" />
</picture>

<details>
<summary>Where the numbers come from, and how I refresh them</summary>

<br />

Sessions live in [data/training.csv](data/training.csv) as `date,minutes`, and the daily workflow re-renders the card from it.

Hevy exports one row per set, so [tools/import_hevy.py](tools/import_hevy.py) collapses rows sharing a start and end time into a single session:

```bash
python3 tools/import_hevy.py ~/Downloads/workouts.csv --dry-run
```

It reports what would change before writing, drops sessions with a zero or overnight duration instead of letting a forgotten timer skew the average, and keeps only the date and the length. Exercise names, weights, reps and workout notes stay out of the repository.
